...
| Word              | `gb`           | Adds another cursor on the next word it finds which is the same as the word under the cursor. First `gb` shows the selection, second time `gb` it creates the actual cursor. Each time you type `gb` you add a new cursor to the next match in the file. `gb` is a command, which means that `14gb` will add a cursor to the next `14` matches in the file. **VS Code specific**                                              |
